...
export default function MenuPageClient() {
  ...
  useEffect(() => {
    ...
    if (langParam && translations[langParam]) setLang(langParam);
    setIsLoading(false);
  }, [searchParams]);
        ...
        <div className="flex justify-between items-start mb-2">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h4 className="font-semibold text-gray-900">{item.name[lang]}</h4>
            </div>
            <p className="text-sm text-gray-600 italic mb-2">
              {t.servedWith}: {item.servedWith[lang]}
            </p>
          </div>
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center overflow-hidden">
              <img src="/crystal-plaza-logo.svg" alt="Logo" className="w-full h-full object-cover rounded-full"/>
            </div>
            <div>
          ...
          <Button onClick={handleBackToLanding} variant="outline" className="bg-white/10 hover:bg-white/20 border-white/30 text-white">
            ...
            {category === 'mainCourse' ? (
              <Tabs defaultValue="tasteOfIndia" className="w-full">
                <TabsList className="grid w-full grid-cols-3 bg-blue-100 border border-blue-200">
                  {Object.keys(menuItems.mainCourse).map(tab => (
                    <TabsTrigger key={tab} value={tab} className="data-[state=active]:bg-blue-900 data-[state=active]:text-white">
                      {typeIcons[tab as keyof typeof typeIcons] || ''} {t.mainCourseCategories[tab as keyof typeof t.mainCourseCategories]}
                    </TabsTrigger>
                  ))}
                </TabsList>
                ...
                {Object.keys(menuItems.mainCourse).map(tab => (
                  <TabsContent key={tab} value={tab} className="mt-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {menuItems.mainCourse[tab as keyof typeof menuItems.mainCourse].map((item: any) => (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {menuItems[category as keyof typeof menuItems].map((item: any) => (
                  <MenuItemCard key={item.id} item={item} />
                ))}
              </div>
            )}
